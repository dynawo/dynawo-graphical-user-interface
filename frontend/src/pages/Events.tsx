...
interface TargetsResp { targets: Target[]; total: number; truncated: boolean; network_loaded: boolean; network_file: string | null; dyd_model_count: number; excluded_modelled: number; unsupported: Unsupported[]; dynawo_available: boolean }
// Objects no event can act on: a library whose ports none of the events match,
// or a kind of equipment the catalogue says nothing about. Grouped, since the
// reason belongs to the library or the kind, not to each object.
interface Unsupported { scope: Kind; label: string; count: number; reason: string }
    ...
    <div style={{ maxWidth: 900 }}>
      ...
      <Card title="1 · Object" style={{ marginBottom: 16 }}>
        <Space direction="vertical" size="middle" style={{ width: '100%' }}>
          ...
          {(targets?.unsupported?.length ?? 0) > 0 && (
            <Tooltip title={
              <Space direction="vertical" size={0}>
                {targets!.unsupported.map(u => (
                  <Text key={`${u.scope}-${u.label}`} style={{ color: '#fff', fontSize: 12 }}>
                    {u.label}{u.count > 1 ? ` ×${u.count}` : ''} — {u.reason}
                  </Text>
                ))}
              </Space>
            }>
              <Text type="secondary">
                {targets!.unsupported.reduce((n, u) => n + u.count, 0)} object(s) not listed:{' '}
                {targets!.unsupported[0].label} — {targets!.unsupported[0].reason}
                {targets!.unsupported.length > 1 && ` (+${targets!.unsupported.length - 1} more)`}
              </Text>
            </Tooltip>
          )}
