...
def parse_events_catalogue(content: bytes) -> list[dict]:
    ...
    Each entry: {id, scope, family_label, family_description, equipment_type,
                 label, lib, for_libs, automatic, destination, patterns, fixed_parameters}
    ...
    for family in root.findall("eventFamily"):
        ...
        for ev in family.findall("event"):
            ...
            events.append({
                ...
                "lib":                ev.get("lib", ""),
                # Libraries of the target this event is reserved for, if any:
                # the way to describe a model that names its ports its own way,
                # without loosening the fragment every other model relies on.
                "for_libs":           [l.strip() for l in ev.get("forLib", "").split("|") if l.strip()],
...
def event_covers_lib(event: dict, lib: str) -> bool:
    """Whether an event may act on a model of this library.

    An event with no forLib covers every library: what it can act on is decided
    by its patterns. One with forLib covers only those named, which is how a
    library whose ports are spelled differently gets an entry of its own without
    that spelling being offered to models that do not have it."""
    return not event["for_libs"] or lib in event["for_libs"]
...
def match_variables(pattern: str, variables: list[str]) -> list[str]:
    """Every variable matching `pattern`, closest match first.
    ...
    A pattern may offer alternatives separated by "|", tried in order: one
    library's port is another's under a different name, and listing the names a
    catalogue entry accepts is what lets it cover both without a second entry.
    Every alternative's matches are kept, those of the earlier one first, so the
    order is still "closest first" but reading "first alternative first".
    """
    matches: list[str] = []
    for fragment in (f.strip() for f in pattern.split("|")):
        if not fragment:
            continue
        low = fragment.lower()
        for v in sorted((v for v in variables if low in v.lower()), key=lambda v: _rank(v, fragment)):
            if v not in matches:
                matches.append(v)
    return matches
...
def explain_unresolved(
    ...
    """Why `event` cannot be wired to an object, in one sentence.

    The information exists when the list of objects is built and was thrown
    away: a model simply did not appear, leaving nothing to explain why."""
    t1, t2 = var1_types or {}, var2_types or {}
    for pattern in event["patterns"]:
        match = match_connection(pattern, var1_variables, var2_variables, t1, t2)
        if match["resolved"]:
            continue
        if not match["var1_matches"]:
            return f'{event["lib"]} has no “{pattern["var1"]}” port'
        if not match["var2_matches"]:
            return f'no “{pattern["var2"]}” port'
        wanted = {t1.get(v) for v in match["var1_matches"] if t1.get(v)}
        kind = f' of type {"/".join(sorted(wanted))}' if wanted else ""
        return f'no “{pattern["var2"]}” port{kind}'
    return ""
